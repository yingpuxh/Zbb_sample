...
namespace Rivet {
  ...
  public:
	...
	_h_toppt          = bookHisto1D("toppt",              26,  200.,  1500.);
      ...
      _h_fatjetptafter    = bookHisto1D("fatjetptafter",           26,  200.,  1500.);
      _h_fatjeteta        = bookHisto1D("fatjeteta",          25,  -2.5,    2.5);
      _h_fatjetetaafter   = bookHisto1D("fatjetetaafter",          25,  -2.5,    2.5);
      _h_fatjetmass       = bookHisto1D("fatjetmass",         14,    0.,   280.);
      _h_fatjetmassafter  = bookHisto1D("fatjetmassafter",         14,    0.,   120.);
...
//      _h_met2->fill(met.mod()/GeV,         event.weight());
	...
	if (partontops.size() >= 2) if ( partontops[1].perp()>250 * GeV ) _h_toppt->fill(partontops[1].perp()/GeV,  event.weight());
      ...
      _h_fatjetmassafter->fill(trimmedfatjets[0].m()/GeV,                                      event.weight());
      _h_fatjetptafter->fill(trimmedfatjets[0].perp()/GeV,                                     event.weight());
      _h_fatjetetaafter->fill(trimmedfatjets[0].eta(),                                         event.weight());
    ...
    void finalize() {
      ...
      _h_fatjetptafter->scaleW(norm);
      _h_fatjeteta->scaleW(norm);
      _h_fatjetetaafter->scaleW(norm);
      _h_fatjetmass->scaleW(norm);
      _h_fatjetmassafter->scaleW(norm);
...
//      _h_met0->scaleW(norm);
  ...
  private:
    ...
    Histo1DPtr   _h_fatjetptafter;
    Histo1DPtr   _h_fatjeteta;
    Histo1DPtr   _h_fatjetetaafter;
    Histo1DPtr   _h_fatjetmass;
    Histo1DPtr   _h_fatjetmassafter;
